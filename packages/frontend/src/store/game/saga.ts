...
import { sendAnswer, sessionCreated, receiveMessage, startNewGame } from './slice';
...
function* newGameSaga(): Generator {
  const response: any = yield call(fetch, 'http://192.168.0.103:3001/session', { method: 'POST' });
  const { id }: any = yield call([response, 'json']);

  yield put(sessionCreated(id));

  const ws = new WebSocket(`ws://192.168.0.103:3002?id=${id}`);
  yield call(() => new Promise<void>((resolve) => ws.addEventListener('open', () => resolve())));
