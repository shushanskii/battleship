...
export const initShip = (origin: string, direction: ShipDirection, length: number): Ship => ({
  origin, direction, length, hits: [],
})
...
export const shipHit = (ship: Ship, deckIndex: number): void => {
  if (!ship.hits.includes(deckIndex)) ship.hits.push(deckIndex)
}

export const shipIsSunk = (ship: Ship): boolean => ship.hits.length >= ship.length
