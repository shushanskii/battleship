import { Ship, shipCoords } from './ship'
...
export const boardSetStatus = (board: Board, index: string, status: CellStatus): void => {
  if (board.cells[index]) board.cells[index].status = status
}

export const boardClone = (board: Board): Board => ({
  size: board.size,
  cells: Object.fromEntries(Object.entries(board.cells).map(([k, v]) => [k, { ...v }])),
})
...
export const boardPlace = (board: Board, ship: Ship): void => {
  if (!boardCanPlace(board, ship)) throw new Error(`Cannot place ship at ${ship.origin}`)
  shipCoords(ship).forEach(index => boardSetStatus(board, index, CellStatus.SHIP))
}
...
export const boardPrint = (board: Board, ships: Ship[] = []): string => {
  ...
  for (const ship of ships) {
    shipCoords(ship).forEach((index, deckIndex) => {
      boardSetStatus(snapshot, index, ship.hits.includes(deckIndex) ? CellStatus.HIT : CellStatus.SHIP)
    })
  }

  const colLabels = Array.from({ length: board.size }, (_, x) => indexToLabel(x))
  ...
  const dataRows = Array.from({ length: board.size }, (_, y) => {
    ...
    const cells = colLabels.map(col => fmt(CellSymbol[snapshot.cells[`${col}${y + 1}`]!.status]))
